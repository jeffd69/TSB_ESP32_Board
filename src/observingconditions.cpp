...
void ObservingConditions::aReadJson(JsonObject &root)
{
    ...
    if(JsonObject obj_config = root[F("Configuration")]) {
        _avgperiod  = obj_config[F("Average_Period")] | _avgperiod;
        _refresh    = obj_config[F("Refresh_Period")] | _refresh;
    }
}
...
void ObservingConditions::aWriteJson(JsonObject &root)
{
    ...
    obj_config[F("Average_Period")]  = _avgperiod;
    obj_config[F("Refresh_Period")]  = _refresh;
